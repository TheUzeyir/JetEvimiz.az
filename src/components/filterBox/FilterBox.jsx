  ...
  const handleFilter = () => {
    ...
    fetch(fullUrl)
      ...
      .then((data) => {
        ...
        setFilteredResults(data.data?.items || []); // Update filtered results in local state
        setFilteredProducts(data.data?.items || []); // Update filtered products in parent component
      })
      .catch((error) => {
              ...
              <div key={param.parameterId} className={style.filterBox_content}>
                {param.parameterTypeId === 2 && param.parameterKey.includes("min") && (
                  <div className={style.filterLabel}>
                    <label htmlFor={`param-${param.parameterId}`}>{param.parameterTitle}</label>
                    <input
                      type="number"
